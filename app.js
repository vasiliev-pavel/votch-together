...
// Демонстрация экрана недоступна на мобильных браузерах.
...
function parseUrl(raw) {
  ...
  // Прямой видеофайл — по расширению.
  if (/\.(mp4|webm|ogg|ogv|mov|m4v|m3u8)(\?|#|$)/i.test(url)) return { type: 'file', url };
  // Ссылка на страницу сайта — встроить и синхронизировать плеер нельзя.
  if (/^https?:\/\/[^/]+\.[^/]/i.test(url)) return { type: 'site', url };
...
// play() может вернуть promise (файл) или undefined (YouTube); глушим отказ.
function playSafe() {
  const p = controller && controller.play();
...
function applyRemote(type, time) {
  ...
  withRemote(() => {
    if (type === 'play')      { controller.seek(time); promptIfBlocked(controller.play(), time); }
    else if (type === 'pause'){ controller.pause();    controller.seek(time); }
    else if (type === 'seek') { controller.seek(time); }
  });
}

// Мобильные браузеры блокируют автозапуск без касания — показываем кнопку «нажми».
function promptIfBlocked(playPromise, time) {
  if (playPromise && typeof playPromise.then === 'function') {
    playPromise.catch(() => showTapToPlay(time));
  } else {
    // YouTube play() не возвращает promise — проверяем результат чуть позже.
    setTimeout(() => { if (controller && !controller.isPlaying()) showTapToPlay(time); }, 1200);
  }
}

function showTapToPlay(time) {
  if (els.wrap.querySelector('.tap-overlay')) return;
  const ov = document.createElement('div');
  ov.className = 'tap-overlay';
  ov.innerHTML = '<div class="tap-btn">▶ Нажмите, чтобы смотреть синхронно</div>';
  ov.addEventListener('click', () => {
    withRemote(() => { controller.seek(time); playSafe(); });
    ov.remove();
  });
  els.wrap.appendChild(ov);
}
...
function applyBeat(time, playing) {
  ...
  withRemote(() => {
    ...
    if (playing && !controller.isPlaying()) playSafe();
...
els.load.addEventListener('click', () => {
  ...
  if (src.type === 'site') { hintUseScreenShare(); return; }
  setSource(src, { time: 0, playing: false });
  send({ t: 'src', src });
});

// Пользователь вставил ссылку на сайт (а не на видео).
function hintUseScreenShare() {
  if (!canScreenShare) {
    // Телефон: демонстрации экрана нет — объясняем, что доступно.
    showPlaceholder(
      'Это адрес <b>сайта</b>, а не видеофайл. На телефоне открыть такой сайт синхронно нельзя.<br>' +
      'Используйте ссылку <b>YouTube</b> или прямую ссылку на видеофайл. ' +
      'Демонстрация экрана для сторонних сайтов работает только с компьютера.'
    );
    return;
  }
  showPlaceholder(
    'Это адрес <b>сайта</b>, а не видеофайл — встроить и синхронизировать его плеер нельзя.<br>' +
    'Чтобы смотреть такой сайт вместе, нажмите <b>«🖥 Поделиться экраном»</b> и выберите вкладку с видео.'
  );
  setScreenStatus('⬅ нажмите, открыв сайт в соседней вкладке');
  els.shareScreen.classList.remove('attention');
  void els.shareScreen.offsetWidth; // перезапустить анимацию
  els.shareScreen.classList.add('attention');
  els.shareScreen.scrollIntoView({ behavior: 'smooth', block: 'center' });
}
...
/* ================================ Старт ================================== */
// На устройствах без захвата экрана (телефоны) прячем кнопку трансляции.
if (!canScreenShare) {
  els.shareScreen.style.display = 'none';
  setScreenStatus('На телефоне доступны YouTube и прямые ссылки на видео.');
}
